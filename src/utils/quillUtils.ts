...
export const searchUserMention = (channelUserList: UserType[], search: string) => {
  const matchedUsers: UserType[] = [];
  const searchList = search.toLocaleLowerCase().split(" ");

  for (const channelUser of channelUserList) {
    const nameList = channelUser.name.toLocaleLowerCase().split(" ");
    let n = 0;
    let s = 0;
    let nIdx = 0;
    let sIdx = 0;
    ...
    if (!searchList[s] || !searchList[s][sIdx]) matchedUsers.push(channelUser);
  }
